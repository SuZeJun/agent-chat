...
- Go 源码注释默认使用中文，通用技术术语保留英文，例如 Context、advisory lock、embedding、trace 和 idempotency。
- 新增或修改的跨包导出标识符，应提供以标识符名称开头的文档注释。
- 注释重点解释设计原因、调用契约、业务约束、并发语义、安全边界和兼容性处理。
- 不得添加复述代码行为的逐行注释，例如“创建变量”“执行查询”“返回结果”。
- 以下逻辑存在非显而易见行为时必须注释：
  - 事务边界与 advisory lock
  - 幂等、重试和状态转换
  - goroutine 生命周期与 Context 取消
  - 权限、安全和隐私处理
  - 跨平台兼容与临时 workaround
  - Eino Graph 路由和中断恢复条件
- 修改实现时必须同步更新或删除失效注释。
- TODO 必须说明未完成原因，并关联 Issue；不得保留没有上下文的 TODO。
- 产品说明放在 README，架构和设计决策放在 `docs/`，局部实现原因才放在源码注释中。
...
- `main` 必须始终保持可构建、可测试。
- 除仓库初始化或用户明确要求外，功能和修复使用短生命周期分支：
  - `feat/<name>`
  - `fix/<name>`
  - `docs/<name>`
  - `chore/<name>`
- 不得直接在 `main` 上进行实验性开发。
- 不得对 `main` 执行 force push。
...
- 使用 Conventional Commits：
  ...
  - `ci`：CI/CD
  - `chore`：工程维护
- 格式为：`<type>(可选 scope): <中文摘要>`。
- 示例：
  - `feat(rag): 实现 FAQ 向量检索`
  - `fix(ci): 修复 pgvector 集成测试并发竞态`
  - `docs: 补充本地开发说明`
- 每个提交只包含一个逻辑完整的改动。
- 功能对应的测试和必要文档应与功能放在同一提交中。
- 不得使用 `update`、`fix bug`、`修改代码` 等无法说明意图的信息。
- 不得提交 WIP、调试代码、临时日志或被注释掉的旧实现。

### 19.3 提交前检查

- 检查 `git status` 和 staged diff，确认没有无关文件。
- 优先暂存明确路径；使用 `git add .` 前必须确认 `.gitignore` 和工作区状态。
- 不得提交 `.env`、密钥、Token、缓存、构建产物、覆盖率文件和 IDE 配置。
- 运行与改动相关的测试；数据库、迁移和启动流程变更必须运行真实数据库集成测试。
- 推送后检查 GitHub Actions；CI 失败必须定位根因，不能通过删除测试或降低安全约束绕过。

### 19.4 Agent 权限

- 未经用户明确要求，不得执行 commit、push、merge、rebase、reset、tag 或 force push。
- 已推送的提交默认不得 amend 或改写历史；修复应创建新提交。
- 执行 Git 写操作前，必须向用户说明将要提交的文件和提交信息。
